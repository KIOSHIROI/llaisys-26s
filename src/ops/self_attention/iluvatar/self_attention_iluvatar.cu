...
template <typename T>
__device__ __forceinline__ float attention_score(const T *q_row, const T *k, size_t nkvhead,
...
template <typename T>
__global__ void self_attention_kernel_iluvatar(T *attn_val, const T *q, const T *k, const T *v,
    ...
    for (size_t j = 0; j < total_len; ++j)
        max_score = fmaxf(max_score, attention_score(q_row, k, nkvhead, kv_h, j, d, scale, past_limit));

    // pass 2: sum of exp (ascending)
    float sum = 0.0f;
    for (size_t j = 0; j < total_len; ++j)
        sum += expf(attention_score(q_row, k, nkvhead, kv_h, j, d, scale, past_limit) - max_score);
    ...
    for (size_t r = 0; r < dv; ++r) {
        float acc = 0.0f;
        for (size_t j = 0; j < total_len; ++j)
            acc += expf(attention_score(q_row, k, nkvhead, kv_h, j, d, scale, past_limit) - max_score)
...
void self_attention(std::byte *attn_val, const std::byte *q, const std::byte *k, const std::byte *v,
    ...
    DISPATCH_DTYPE(type, T, {
        self_attention_kernel_iluvatar<T><<<(int)((total + kThreads - 1) / kThreads), kThreads>>>(
