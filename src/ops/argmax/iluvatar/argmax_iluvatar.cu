...
// first (lowest index) wins, max_val is stored in the element type.
template <typename T>
__global__ void argmax_kernel_iluvatar(int64_t *max_idx, T *max_val, const T *vals, size_t numel) {
    size_t idx = 0;
...
void argmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals,
    ...
    DISPATCH_DTYPE(type, T, {
        argmax_kernel_iluvatar<T><<<1, 1>>>(reinterpret_cast<int64_t *>(max_idx),
