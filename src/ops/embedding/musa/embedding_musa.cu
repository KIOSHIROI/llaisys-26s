#include "embedding_musa.hpp"

#include "../../common/dispatch_musa.cuh"

// Embeds a single token: index[0] lives on device (int64_t).
template <typename T>
__global__ void embedding_kernel_musa(T *out, const int64_t *index, const T *weight, size_t numel) {
...
void embedding(std::byte *out, const std::byte *index, const std::byte *weight,
    ...
    DISPATCH_DTYPE_MUSA(type, T, {
        embedding_kernel_musa<T><<<(int)((numel + kThreads - 1) / kThreads), kThreads>>>(
