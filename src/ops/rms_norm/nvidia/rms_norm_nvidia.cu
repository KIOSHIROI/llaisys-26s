...
template <typename T>
__global__ void rms_norm_sumsq_kernel(const T *in, float *sumsq, size_t u, size_t v) {
...
void rms_norm(std::byte *out, const std::byte *in, const std::byte *weight, float eps,
    ...
    DISPATCH_DTYPE(type, T, {
        rms_norm_sumsq_kernel<T><<<(int)((u + kThreads - 1) / kThreads), kThreads>>>(
            reinterpret_cast<const T *>(in), d_sumsq, u, v);
        rms_norm_apply_kernel<T><<<(int)((u * v + kThreads - 1) / kThreads), kThreads>>>(
